...
export class NoteHeaderActions {
    ...
    BACK_BTN = By.css('a[data-testid="btn-exit-note"]');
    DROPDOWN_SAVE_CONTAINER = By.id('dropdown-save');
    DROPDOWN_PUBLISH_CONTAINER = By.id('dropdown-publish');
    // Nota: Estos IDs compartidos son un riesgo potencial en el DOM.
    SAVE_AND_EXIT_OPT = By.id("option-dropdown-0");
    EXIT_WITHOUT_SAVING_OPT = By.id("option-dropdown-1");
    PUBLISH_AND_EXIT_OPT = By.id("option-dropdown-0");
    SCHEDULE_OPT = By.id("option-dropdown-1");
    ...
    MODAL_PUBLISH_CONFIRM_BTN = By.css('app-cmsmedios-button[data-testid="post-note-confirm"] button[data-testid="btn-calendar-confirm"]');
    locatorMap = {
     ...
     */
    async clickExitAction(action, opts = {}) {
        const config = {
            ...DefaultConfig,
            ...opts,
...
//# sourceMappingURL=NoteHeaderActions.js.map