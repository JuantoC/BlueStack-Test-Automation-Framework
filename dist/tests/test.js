...
(async function basicAuth() {
    ...
    options.addArguments('--no-first-run', '--no-default-browser-check', '--disable-extensions', '--disable-default-apps', '--incognito', '--start-maximized', '--force-device-scale-factor=0.8');
    ...
    try {
        ...
        let do_it_later_btn = await driver.wait(until.elementLocated(By.css('[data-testid="btn-next"]')), 3000);
        await driver.wait(until.elementIsVisible(do_it_later_btn), 3000);
        await driver.wait(until.elementIsEnabled(do_it_later_btn), 3000);
        await driver.sleep(200);
        await do_it_later_btn.click();
        console.log('Will do it later btn - ok');
        // Create New Post
        let create_post_btn = await driver.wait(until.elementLocated(By.css("button.btn-create-note")), 6000);
        ...
        let new_post = await driver.wait(until.elementLocated(By.css('#option-dropdown-0 label')), 3000);
        await driver.wait(until.elementIsVisible(new_post), 3000);
