runSession(
  ...
  async ({ driver, opts, log }) => {
    ...
    description(`
### Test: Flujo completo de Youtube Video
---
**Objetivo:** Validar la creación de un Youtube desde cero, la navegación al componente de videos, y la edición inline del título del Youtube previamente creado antes de su publicación final.

**Flujo de pasos:**
1. Login como editor.
2. Navegación al componente de Videos vía sidebar.
3. Creación de nueva nota tipo Youtube con datos frescos.
4. Edición inline del título del Youtube creado en el paso 3.
5. Busqueda y publicacion del ultimo video de la tabla.

> **Resultado esperado:** El Youtube se crea, el video se sube correctamente, y luego el Youtube se edita y publica exitosamente.
`);
    ...
    const youtubeContainer = await video.table.getVideoContainerByTitle(newYoutubeData.title!);
    await video.changeVideoTitle(youtubeContainer);
