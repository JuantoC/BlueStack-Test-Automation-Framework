...
export class NoteListicleSection {
  ...
  private readonly ADD_OPTION_BTN = By.id('option-dropdown-0');
   ...
   */
  async getListicleItemState(index: number, opts: RetryOptions = {}): Promise<'expanded' | 'collapsed'> {
  ...
  async ensureItemExpanded(index: number, opts: RetryOptions = {}): Promise<void> {
   ...
   */
  async fillListicleItems(items: Array<{ title: string; body: string }>, opts: RetryOptions = {}): Promise<void> {
