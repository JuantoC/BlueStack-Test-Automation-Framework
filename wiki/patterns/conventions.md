...
## Patrón constructor — Maestro estándar (sin `NoteType` fijo)

`MainPostPage` no recibe `noteType` en el constructor — el tipo se pasa en cada llamada a `createNewNote(noteType)`. Esto permite que una sola instancia gestione POST, LISTICLE, LIVEBLOG y AI_POST sin reinstanciar el PO.
...
Aplica a: `MainPostPage`, `MainVideoPage`, `MainImagePage`, `MainTagsPage` y todos los Maestros actuales del framework.
...
> Ver formatos operativos de extensión y placeholders en [pom-generator — references/pom-formats.md](.claude/skills/pom-generator/references/pom-formats.md).
